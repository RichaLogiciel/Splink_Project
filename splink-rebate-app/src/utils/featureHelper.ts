...
import { getOrderingFeaturePreference } from "./orderingFeaturePreference";

/**
 * FUNCTION TO ENABLE ORDERING FEATURE FOR SALES REP IDS (Client-side)
 *
 * Logic flow:
...
const enableOrderingFeature = (
  ...
  if (
    ...
    ORDERING_FEATURE_EXCLUDED_WAREHOUSE_IDS.includes(primaryWarehouseId)
  ) {
    return false;
  }
  ...
  return (
    !ORDERING_FEATURE_EXCLUDED_SALES_REP_IDS.includes(associatedUserId) &&
    ORDERING_FEATURE_ENABLED
  );
};

export { enableOrderingFeature };
