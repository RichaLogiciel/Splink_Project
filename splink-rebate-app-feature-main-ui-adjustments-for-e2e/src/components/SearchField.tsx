...
const SearchFieldInput: React.FC<SearchFieldProps> = ({ ...props }) => {
  ...
  useEffect(() => {
    ...
    // Handle when URL parameter exists and differs from current state
    if (
      urlSearchParam &&
    ...
    // Handle when URL parameter is removed but local state still has a value
    // This ensures cross-tab synchronization when search is cleared in another tab
    else if (!urlSearchParam && (searchInput || debouncedSearch)) {
      setSearchInput("");
      setDebouncedSearch("");
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams]); // Watch for URL changes only
