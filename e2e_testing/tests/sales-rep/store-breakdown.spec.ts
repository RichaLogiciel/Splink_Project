...
salesRepMap.forEach((salesRep, salesRepKey) => {
  ...
  test.describe(`Store Page Detail: ${salesRep.name}`, () => {
    let usersTable;
    let storePage;
    ...
    // This test verifies the Store Breakdown page for each store under a salesRep.
    test('Test and Verify Purchase Volume and Estimated Earnings for each store on Stores Page', async ({
      ...
      console.log(`========= Page Started for: ${salesRep.name} =========`);
      await expectWithMessage(
        () =>
      ...
      for (let i = 0; i <= maxStores; i++) {
        ...
        console.log(
          `--------- Testing Store: ${row.store} for: ${salesRep.name} ---------`
        );
        ...
        if (!(await currentStoreRow.isVisible())) {
          ...
          errorFlag.push({
            failed: true,
            message: `Store: ${row.store} not found in the Store Table for: ${salesRep.name}.\nScreenshot saved: ${screenshot}\n`,
          });
          console.log(
            `Store: ${row.store} not found in the Store Table for: ${salesRep.name}`
          );
          continue;
        }
        ...
        if (!isNotEnrolledTabVisible) {
          const screenshot = await saveScreenshot(
            page,
            `not-enrolled-tab-not-visible-${salesRep.name}`
          );
          errorFlag.push({
            failed: true,
            message: `Not Enrolled Tab is not visible for Sales Rep: ${salesRep.name} in Store Details: ${row.store}\n\n
            Screenshot saved: ${screenshot}\n`,
          });
          continue;
        }
        ...
        if (
          ...
          const screenshot = await saveScreenshot(
            page,
            `total-purchase-volume-not-matching-${salesRep.name}-${row.store}`
          );
          errorFlag.push({
            failed: true,
            message: `For Sales Rep: ${salesRep.name} | Store Name: ${
              row.store
          ...
          console.log(
            ...
            For Sales Rep: ${salesRep.name} | Store Name: ${row.store}\n`
          );
        }
        ...
        if (
          ...
          const screenshot = await saveScreenshot(
            page,
            `total-estimated-earnings-not-matching-${salesRep.name}-${row.store}`
          );
          errorFlag.push({
            failed: true,
            message: `For Sales Rep: ${salesRep.name} | Store Name: ${
          ...
          console.log(
            `Total Estimated Earnings: ${totalEstimatedEarnings} | Sum of Enrolled Programs: ${
              dashboardMetrics.estimatedEarnings
            } | Difference: ${
              totalEstimatedEarnings - dashboardMetrics.estimatedEarnings
            }
          ...
          for (let j = 0; j < maxPrograms; j++) {
            ...
            const isCompletedSkus =
              await storeDetailModal.matchCompletedSkusTiersInStoreDetailsModal(
                {
                  loggedInUser: salesRep.name,
                  manufacturer: program.manufacturer,
                  store: row.store,
                }
              );
            console.log(
              `isCompletedSkus: ${isCompletedSkus} | from modal: ${program.programCompliance.completed} | from table: ${row.programCompliance.completed} for ${salesRep.name} - ${program.manufacturer} - ${row.store}`
            );
            ...
            if (
              ...
              const screenshot = await saveScreenshot(
                page,
                `completed-skus-not-matching-${row.store}-${program.manufacturer}`
              );
              errorFlag.push({
                failed: true,
                message: `Completed Skus are not matching for ${salesRep.name} - ${program.manufacturer} Store: ${row.store}\nTier: ${row.tier} | Completed Skus: ${isCompletedSkus} | Expected: ${program.programCompliance.completed}\nScreenshot: ${screenshot}\n\n`,
              });
            }
            ...
            try {
              ...
              const screenshot = await saveScreenshot(
                page,
                `tier-verification-failed-${salesRep.name}-${row.store}-${program.manufacturer}`
              );
              errorFlag.push({
                failed: true,
                message: `Tier verification failed for ${salesRep.name} - ${row.store} - ${program.manufacturer}\n${error.message}\nScreenshot: ${screenshot}\n\n`,
              });
            }
      ...
      console.log(`========= Page Completed for: ${salesRep.name} =========`);
