...
import ProgramPage from '../../pages/sales-rep/ProgramPage';
import StoreProgramDetailPage from '../../pages/sales-rep/StoreProgramDetailPage';
...
// Helper to load JSON for a salesRep
function loadSalesRepJson(jsonPath) {
...
salesRepMap.forEach((salesRep, salesRepKey) => {
  ...
  test.describe(`Store Program Detail: ${salesRep.name}`, () => {
    let programPage;
    let usersTable;
    let detailPage;
    let outputData;
    ...
    test.beforeEach(async ({ page, browserName }) => {
      ...
      await page.waitForLoadState('networkidle');
      // Load the correct JSON for this salesRep
      outputData = loadSalesRepJson(salesRep.jsonPath);
    });

    // This test verifies the Retailer Program Detail page for each program under a salesRep.
    // It checks:
    ...
    test('Overview Tab: Retailer Program for each program', async ({
      ...
      for (let i = 0; i < allPrograms.length; i++) {
        ...
        console.log(
          `========= Page Started for: ${salesRep.name} - ${program.manufacturerName} =========`
        );
        ...
        console.log(
          `Estimated Earnings: ${estEarnings} | Expected: ${program.estimatedEarnings}`
        );
        console.log(
          `Sales Volume: ${salesVolume} | Expected: ${program.purchaseVolume}`
        );
        ...
        expect(estEarnings).toBe(
          ...
          `Expected Est Earnings: ${program.estimatedEarnings}, Actual Est Earnings: ${estEarnings} not matching for Distributor: ${salesRep.name} and Manufacturer: ${program.manufacturerName}`
        );

        // Compare Sales Volume with Program overview
        expect(salesVolume).toBe(
          program.purchaseVolume,
          `Expected Sales Volume: ${program.purchaseVolume}, Actual Sales Volume: ${salesVolume} not matching for Distributor: ${salesRep.name} and Manufacturer: ${program.manufacturerName}`
        );

        for (const retailer of retailerBreakdown) {
          if (retailer == null) continue;
          const row = page.locator(
            `${detailPage.selectors.storeRetailerProgramRow}:has-text("${retailer.type}")`,
            { timeout: 1000 }
          );
          ...
          if (totalStoresInRow != totalStores) {
            const screenshot = await saveScreenshot(
              page,
              `total-stores-not-matching-${salesRep.name}-${program.manufacturerName}-${retailer.type}`
            );
            await softExpectWithScreenshot(
              page,
              () => expect.soft(totalStoresInRow).toBe(totalStores),
              `Total stores in row should match total stores from dashboard card for ${salesRep.name} - ${program.manufacturerName} - ${retailer.type}\nTotal Stores in Row: ${totalStoresInRow} | Total Stores from Dashboard Card: ${totalStores}\nScreenshot: ${screenshot}\n\n`
            );
          }

          console.log(
            `Total Stores for current row: ${totalStoresInRow}, Total Stores from Dashboard Card: ${totalStores} for ${salesRep.name} - ${program.manufacturerName} - ${retailer.type}`
          );
          ...
          if (classAttr && classAttr.includes('cursor-pointer')) {
            ...
            await softExpectWithScreenshot(
              ...
              () =>
                expect
                  ...
                  .toBeVisible(),
              `Expected Retailer Program Modal to be visible after clicking row for ${salesRep.name} - ${program.manufacturerName} - ${retailer.type}`
            );
            ...
            const complianceStatus = await page
    ...
    test('Enrolled Stores and Stores Not Enrolled Tab: Test and verify each store modal and its tier', async ({
      ...
      for (let i = 0; i < allPrograms.length; i++) {
        ...
        if (
          ...
          console.log(
            `Skipping ${salesRep.name} - ${program.manufacturerName} as it is in the skip list`
          );
          continue;
        }
        console.log(
        ...
        for (const tab of tabs) {
          ...
          if (tabText.includes('Stores')) {
            ...
            isTabEnabled = true;
            console.log(
              `---------------- Switched to Stores tab for ${salesRep.name} - ${program.manufacturerName} ----------------`
            );
          }
          ...
          if (tabText.includes('Stores Not Enrolled')) {
            ...
            isTabEnabled = true;
            console.log(
              `---------------- Switched to Stores Not Enrolled tab for ${salesRep.name} - ${program.manufacturerName} ----------------`
            );
          }
          ...
          if (!isTabEnabled) {
            ...
            console.error(
              `Stores or Stores Not Enrolled tab not found for ${salesRep.name} - ${program.manufacturerName}`
            );
            expect
          ...
          console.log(
            `For ${salesRep.name} - ${program.manufacturerName} | Max Stores to Test: ${maxStores} | Total Stores: ${rows.length}`
          );
          for (let i = 0; i < maxStores; i++) {
            ...
            console.log(
              `---------------- Working on ${salesRep.name} - ${program.manufacturerName} Store: ${row.store} ----------------`
            );
            ...
            if (
              ...
              console.error(
                `Completed Skus are not matching for ${salesRep.name} - ${program.manufacturerName} Store: ${row.store} tier: ${row.tier} | Completed Skus: ${isCompletedSkus} | Expected: ${row.programCompliance.completed}`
              );
              await softExpectWithScreenshot(page, () =>
                expect
            ...
            for (const tier of tierElements) {
              ...
              for (const category of skuCategory) {
                ...
                if (await categoryTabBtn.isVisible()) {
                  ...
                  console.error(
                    `Category Tab Button is not visible for ${salesRep.name} - ${program.manufacturerName} Store: ${row.store} - ${categoryName}`
                  );
                  if (isPurchasedCheckboxVisible) {
                ...
                if (isPurchasedCheckboxVisible) {
                  ...
                  await softExpectWithScreenshot(
                    ...
                    `Expected number of purchased products to be greater than or equal to completed SKUs for Category: ${categoryName} | Tier Progress Track: ${completed}/${total} | Purchased Products: ${purchasedSkus} for Distributor: ${salesRep.name} - ${program.manufacturerName} Store: ${row.store}`
                  );
                } else {
                  console.error(
                    `Purchased Checkbox is not visible, Category: ${categoryName} | Tier Progress Track: ${completed}/${total} for Distributor: ${salesRep.name} - ${program.manufacturerName} Store: ${row.store}`
                  );
                }
              }
            ...
            await softExpectWithScreenshot(
              ...
              () =>
                expect
                  ...
                  .toBe(row.programCompliance.completed),
              `Completed Tiers should match the completed tiers in the row for Distributor: ${salesRep.name} - ${program.manufacturerName} Store: ${row.store}`
            );
            ...
            await softExpectWithScreenshot(
              ...
              `Pending Tiers should match the total tiers minus completed tiers for Distributor: ${salesRep.name} - ${program.manufacturerName} Store: ${row.store}`
            );
          ...
          console.log(
            `================= Page Completed for: ${salesRep.name} - ${program.manufacturerName} ==================`
          );
        }
