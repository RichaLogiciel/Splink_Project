    ...
    await queryInterface.sequelize.query(`
      ...
      CASE
	        ...
	        CASE
	            ...
	            ELSE 0
	        END
			WHEN pro_d.rebate_type::text = 'per_category_item'::text THEN
			  SUM(
			    COALESCE(
			      (
			        SELECT SUM(li.total_units * rebate_val::numeric)
			        FROM jsonb_array_elements_text(p.category_tags_json) AS tag(tag_value)
			        JOIN LATERAL (
			          SELECT trim(ft.category) as category, ft.idx,
			                 (string_to_array(pro_d.fixed_rebate_amount, ','))[ft.idx] AS rebate_val
			          FROM unnest(string_to_array(pro_d.fixed_rebate_category, ',')) WITH ORDINALITY AS ft(category, idx)
			        ) AS matched ON matched.category = tag.tag_value
			      ),
			      0
			    )
			  )
