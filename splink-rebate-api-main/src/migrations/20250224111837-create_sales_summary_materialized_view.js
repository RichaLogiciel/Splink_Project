...
module.exports = {
  ...
  down: async (queryInterface) => {
    // Drop the triggers
    await queryInterface.sequelize.query(`
      DROP TRIGGER IF EXISTS line_items_sales_refresh_trigger ON line_items;
    `);

    await queryInterface.sequelize.query(`
      DROP TRIGGER IF EXISTS products_sales_refresh_trigger ON products;
    `);

    // Drop the trigger function
    await queryInterface.sequelize.query(`
      DROP FUNCTION IF EXISTS refresh_sales_summary_materialized_view;
    `);
